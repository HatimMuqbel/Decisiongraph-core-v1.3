...
from decisiongraph.governed_confidence import compute_governed_confidence
...
class PolicySimulator:
    ...
    def simulate(self, draft: DraftShift) -> SimulationReport:
        ...
        for seed in matching_seeds:
            ...
            sev_before = _DISP_SEVERITY.get(orig_disp, 0)
            sev_after = _DISP_SEVERITY.get(sim_disp, 0)
    ...
    def _compute_cascade(
        ...
        For each affected typology:
        1. Count pool dispositions BEFORE (from current seeds)
        2. Apply simulated changes to get pool AFTER
        3. Compute governed confidence for a hypothetical new case
        4. Compare posture before vs after
        5. Flag posture reversals
        """
